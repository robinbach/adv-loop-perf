...
#define SP_DEF 1 // number of repetitions of speedy must be >=1
int SP = SP_DEF;

/* higher ITER --> more likely to get correct # of centers */
/* higher ITER also scales the running time almost linearly */
//#define ITER_DEF 3 // iterate ITER* k log k times; ITER >= 1
#define ITER_DEF 3 // iterate ITER* k log k times; ITER >= 1
int ITER = ITER_DEF;
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal, int pid,
   ...
   while (1) {
      ...
      lastcost = cost;
      cost = pFL(points, feasible, numfeasible, z, &k, cost, (long) (ITER
            * kmax * log((double) kmax)), 0.1, pid, barrier);

      /* if number of centers seems good, try a more accurate FL */
      if (((k <= (1.1) * kmax) && (k >= (0.9) * kmin)) || ((k <= kmax + 2)
            && (k >= kmin - 2))) {
         ...
         /* may need to run a little longer here before halting without
          improvement */
         cost = pFL(points, feasible, numfeasible, z, &k, cost, (long) (ITER
               * kmax * log((double) kmax)), 0.001, pid, barrier);
      }
   ...
   for (i = 0; i < points->num; i++) {
      ...
      if (points->p[i].assign != i) {
         ...
         for (ii = 0; ii < points->dim; ii++) {
            ...
            points->p[points->p[i].assign].coord[ii] += points->p[i].coord[ii]
                  * relweight;
         }
...
public:
   FileStream(char* filename, int maxlines_) {
   ...
   size_t read(float* dest, int dim, int num) {
      ...
      while (size < num) {
         ...
         if (line.size() < 10)
            continue; // hack: get rid of short info lines...
   ...
   int feof() {
      return fp.eof() || fReadLines >= fMaxLines;
      //return std::feof(fp);
   }
   ~FileStream() {
...
void outcenterIDs(Points* centers, long* centerIDs, char* outfile) {
   ...
   fprintf(fp, "## (1) the ID of the center\n"
      "## (2) Weight of the center (sum of all point weights)\n"
      "## (3) Coordinates of the center \n\n");
...
void streamCluster(PStream* stream, long kmin, long kmax, int dim,
   ...
   while (1) {
      ...
      if (stream->ferror() || numRead < (unsigned int) chunksize
            && !stream->feof()) {
         fprintf(stderr, "ferr=%d numread=%d, cs=%d, feof=%d",
               stream->ferror(), numRead, (unsigned int) chunksize,
               stream->feof());
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc < 10) {
      ...
      fprintf(stderr, "  mode:        Mode of the input generator\n");
      fprintf(stderr,
            "  [seed]       The seed for random number generator (optional)\n");
      fprintf(stderr, "\n");
      fprintf(stderr,
   ...
   if (strcmp(infilename, "none") == 0) {
      fprintf(stderr, "File name not specified!!!!!  ABORTING!!!!");
      exit(1);
   } else {
      stream = new FileStream(infilename, n);
   }
